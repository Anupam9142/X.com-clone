...
const EditProfileModal = (props: any) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    const updatedUser = {
      ...
      usersAt: username.length > 1 ? `@${username}` : currentUser?.usersAt,
        ...
        <form onSubmit={handleSubmit}>
          <div className="formNavHeader">
            <div className="flexNavHeader">
              <span onClick={() => props.setEditProfileModal(false)}>
                {<MdClose cursor="pointer" />}{" "}
              </span>
              <h1>Edit profile </h1>
            </div>
            {uploadSuccess ? (
              <p>Image has been uploaded {<BsCheck2 />} </p>
            ) : (
              ""
            )}
          ...
          <div className="editProfileImg">
            <div
              style={{ backgroundImage: `url(${currentUser?.coverPhoto})` }}
              className="coverBg"
            >
              <div className="uploadCoverPhoto">
                ...
                >
                  {<BiCamera fontSize={30} cursor="pointer" />}{" "}
                </label>
                <input
                  onChange={(e: any) => uploadCoverImage(e.target.files)}
                  type="file"
                  id="fileCover"
                  style={{ display: "none" }}
                />
          ...
          <div className="labelContainer">
            <div
              className={
                currentActiveInput == 0 ? "activeInput" : "labelBorder"
              }
            >
              ...
              <input
                ...
                value={username}
                onClick={() => setCurrentActiveInput(0)}
            ...
            <div
              className={
                currentActiveInput == 1 ? "activeInput" : "labelBorder"
              }
            >
              <div className="flexLabelBorder">
                <label>Bio </label>
              ...
              <input
                ...
                value={bio}
                onClick={() => setCurrentActiveInput(1)}
            ...
            <div
              className={
                currentActiveInput == 2 ? "activeInput" : "labelBorder"
              }
            >
              <div className="flexLabelBorder">
                ...
                {currentActiveInput == 2 ? (
                  <span>
                    {" "}
                    {location.length} / {maxLocationLength}{" "}
                  </span>
                ) : (
                  ""
                )}
              ...
              <input
                ...
                value={location}
                onClick={() => setCurrentActiveInput(2)}
            ...
            <div
              className={
                currentActiveInput == 3 ? "activeInput" : "labelBorder"
              }
            >
              <div className="flexLabelBorder">
                <label>Website </label>
                {currentActiveInput == 3 ? (
                  <span>
                    {" "}
                    {links.length} / {maxWebsiteLength}{" "}
                  </span>
                ) : (
                  ""
                )}
              ...
              <input
                ...
                value={links}
                onClick={() => setCurrentActiveInput(3)}
                maxLength={maxWebsiteLength}
                type="text"
                placeholder={currentUser?.links}
              />
            </div>
          </div>
          <div className="dateOfBirthCon">
            <p>
              Birth date{" "}
              {editDateActive ? (
                <span onClick={() => setEditDateActive(false)}>Cancel </span>
              ) : (
                <span onClick={() => setEditDateActive(true)}>Edit </span>
              )}
            </p>
