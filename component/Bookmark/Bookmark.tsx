...
const Bookmark = (props: any) => {
  ...
  const handleRemoveBookmark = async () => {
    ...
    let filtered = bookmarks.filter(
      (bookmark: any) => bookmark.postId !== props.bookmark.postId
    );
  ...
  return (
    <BookmarkStyle>
      <div className="postsContainer">
        <Link
          href={"/users/" + props.bookmark?.username}
          className="profilePicture"
          style={{ backgroundImage: `url(${props.bookmark?.profileDp})` }}
        ></Link>
        <div className="subPostsContainer">
          <div className="flexTweetProfileDetails">
            <div className="tweetProfileDetails">
              ...
              {props.tweet?.newDates == undefined ? (
                <span className="createdAt">
                  {moment(new Date(props.bookmark?.createdAt)).fromNow()}
                </span>
              ) : (
            ...
            <div className="removeModalContainer">
              ...
              {modalActive ? (
                <span onClick={handleRemoveBookmark} className="remove">
                  Remove bookmark{" "}
                </span>
              ) : (
                ""
          ...
          {props.bookmark?.picture?.length > 1 ? (
            <div
              style={{ backgroundImage: `url(${props.bookmark?.picture})` }}
              className="tweet-image"
            ></div>
          ) : (
            ""
          )}
          {props.bookmark?.video?.length > 1 ? (
            <video
              controls
              src={`${props.bookmark?.video}`}
          ...
          <div className="showThread">
            <div
              style={{
                cursor: "pointer",
                backgroundImage: `url(${props.bookmark?.profileDp})`,
              }}
