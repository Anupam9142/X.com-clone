...
const Comments = (props: any) => {
  ...
  const [createdAt, setCreatedAt] = useState<any>(
    moment(new Date(commentId)).fromNow()
  );
  ...
  const handleComment = async (e: any) => {
    e.preventDefault();
    const commentData = {
      username: props.tweetProps?.username,
      usersPic: props.tweetProps?.profileDp,
    ...
    await axios
      .put(`${process.env.NEXT_PUBLIC_BASE_URL}/tweets/comments`, commentData)
      .catch((err) => console.log(err));
    setComments(" ");
    setPicture("");
    setVideo("");
    ...
    props.setTweetProps({
      ...props.tweetProps,
      comments: [...props.tweetProps.comments, commentData].reverse(),
    });
  };
  ...
  const handleAddLike = async () => {
    ...
    await axios
      .put(`${process.env.NEXT_PUBLIC_BASE_URL}/tweets/`, likeData)
      .catch((err) => console.log(err, "Cannot like "));
  };
          ...
          <div className="textAreaContainer">
            {everyOne ? (
              <p>
                ...
                <span>
                  {props.tweetProps?.usersAt.length > 11
                    ? props.tweetProps?.usersAt.slice(0, 11)
                    : props.tweetProps?.usersAt}{" "}
                </span>
              </p>
            ) : (
              ""
            ...
            <div className="flexIcons">
              ...
              {everyOne ? (
                <div className="tweetButton">
                  {successfulUpload || comments.length > 0 ? (
                    <button
                      type={
                        successfulUpload || comments.length > 0
                          ? "submit"
