...
const Chat = (props: any) => {
  ...
  const handleData = async () => {
    ...
    try {
      await waitForId;
      await axios
        .get(`${process.env.NEXT_PUBLIC_BASE_URL}/users/${waitForId}`)
      ...
      <div className="chatCon">
        <div className="subChatCon">
          <div className="heading">
            <div className="headingDetails">
              <div
                className="profileDp"
                style={{ backgroundImage: `url(${userObject?.profilePic})` }}
              ></div>
              <Link href={"/users/" + data.user?.username}>
                <span className="username">{data.user?.username}</span>
              </Link>
            </div>
            <p onClick={handleResize}>
              {props.isMobile ? (
                <IoMdClose cursor="pointer" fontSize="25" />
              ) : (
                <IoInformationCircleOutline fontSize="25" />
              )}
            </p>
          </div>
          <div className="userObject">
            <Link href={"/users/" + userObject?.username}>
              <div
                style={{ backgroundImage: `url(${userObject?.profilePic})` }}
            ...
            <p className="createdAt">
              Joined {moment(userObject?.createdAt).format("MM YYYY")}{" "}
        ...
        <div className="inputCon">
          <div className="inputSpan">
            ...
            />
            {texts.length > 0 || picture !== null || video !== null ? (
              <button onClick={handleSubmit}>
                {<AiOutlineSend color="#1d9aef" className="sendIcon" />}
              </button>
            ) : (
              <button disabled>
                {<AiOutlineSend color="#1d9aef" className="sendIcon" />}
